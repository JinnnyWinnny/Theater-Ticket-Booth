...
class App:
    ...
    def __init__(self,master):
        ...
        self._cart_text = tk.Text(self._frame2, width=screen_width//30, height=screen_height//40, borderwidth=10)
        ...
        self._receipt_btn.config(font=('Tahoma', 13, 'bold'))
        self._receipt_btn.place(x=screen_width / 8, y=screen_height / 1.75)
        ...
        self._no_receipt_btn.config(font=('Tahoma', 13, 'bold'))
        self._no_receipt_btn.place(x=screen_width / 4.5, y=screen_height / 1.75)
        ...
        self._counter_text = tk.Text(self._frame2, width=30, height=10, borderwidth=10)
        ...
        self._save_btn.config(font=('Tahoma', 13, 'bold'))
        self._save_btn.place(x=screen_width / 4, y=screen_height / 1.05)
    ...
    def create_movie_components(self):
        ...
        for n, title in self._movies.items():
            ...
            title_lbl.place(x=screen_width / 4, y=(n + 0.10) * 150)
            ...
            for time, num_seat in time_w_seats_num.items():
                ...
                time_btns = ttk.Button(self._frame1, text=btn_text, command=lambda
                    nst=nst, n=n: self.new_window(n, self._theater.show_nst_time(n, nst), nst), width=10)
                self._time_btn_list[n].append(time_btns)
                time_btns.place(x=140 + (nst * 120), y=(n + 0.3) * 151)
                nst += 1
    ...
    def update(self, i, j, movie, time):
        ...
        self._seat_buttons[i][j].config(highlightbackground="red")
        ...
        if self._theater.check_seats(movie, time, i, j) == 0:
            ...
            self._theater.update_seat(movie, time, i, j)
            self._seat_buttons[i][j].config(highlightbackground="white")
    ...
    def pass_to_cart(self, new_window, clicked_movie, nst_time):
        ...
        if len(self._movie_w_time_in_cart) != 0:
            ...
            for i in range(num_of_tickets):
                ...
                self._cart_text.insert(tk.INSERT, f"{title:10}  {time:5} \n row: "
    ...
    def new_window(self, movie, time, nst_time):
        ...
        close_button.place(x=w / 3, y=h - 30)
